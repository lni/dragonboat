...
func (oc *OrderedCache) get(key interface{}) *Entry {
	entry := oc.pool.Get().(*Entry)
	entry.Key = key
	defer entry.Release(oc.pool)

	if e, ok := oc.llrb.Get(entry).(*Entry); ok {
		return e
	}
	return nil
}
...
func (oc *OrderedCache) del(key interface{}) {
	entry := oc.pool.Get().(*Entry)
	entry.Key = key
	oc.llrb.Delete(entry)
	entry.Release(oc.pool)
}
...
func (oc *OrderedCache) Ceil(key interface{}) (interface{}, interface{}, bool) {
	...
	if e, ok := oc.llrb.Ceil(entry).(*Entry); ok {
...
func (oc *OrderedCache) OrderedDo(f func(k, v interface{})) {
	for e := oc.ll.back(); e != &(oc.ll.root); e = e.prev {
		f(e.Key, e.Value)
	}
}
...
func (ic *IntervalCache) init() {
	ic.tree = NewTree(ExclusiveOverlapper)
}
...
func (ic *IntervalCache) doGet(i Interface) bool {
	e := i.(*Entry)
	if e.ID() == ic.getID {
		ic.getEntry = e
		return true
...
func (ic *IntervalCache) doOverlaps(i Interface) bool {
	e := i.(*Entry)
