...
//     the on-disk state machine. Specify the .so file name and the
...
class OnDiskStateMachine
{
 ...
 protected:
  ...
  virtual LookupResult lookup(const Byte *data, size_t size) const noexcept = 0;
	...
	// reboot. sync() is always invoked with mutual exclusion protection from the
	// batchedUpdate(), prepareSnapshot() and recoverFromSnapshot().
  ...
  virtual PrepareSnapshotResult prepareSnapshot() const noexcept = 0;
	...
	// file on disk. This is a read only method that should never change the state
  ...
  virtual SnapshotResult saveSnapshot(const void *context,
	...
	// a previously saved snapshot captured by the saveSnapshot() on a remote
	// node. The saved snapshot is provided as an SnapshotReader backed by a file
	// already fully available on disk. Dragonboat ensures that the
	// batchedUpdate(), sync(), prepareSnapshot() and saveSnapshot() will not be
	// invoked when recoverFromSnapshot() is in progress.
