...
const (
	...
	// EmptyPayloadClientID is the special client id used when an empty entry
	// is proposed with a NoOP session.
	EmptyPayloadClientID uint64 = 1
	// NoOPClientID is a special client id used for a NoOP session.
...
func NewSession(clusterID uint64, rng random.Source) *Session {
	for {
		...
		if cid != NotSessionManagedClientID &&
			cid != EmptyPayloadClientID && cid != NoOPClientID {
			return &Session{
				ClusterID: clusterID,
				ClientID:  cid,
...
// application.
func NewNoOPSession(clusterID uint64) *Session {
	return &Session{
		ClusterID: clusterID,
...
func (m *Session) IsNoOPSession() bool {
	return m.SeriesID == NoOPSeriesID &&
		(m.ClientID == NoOPClientID || m.ClientID == EmptyPayloadClientID)
}
...
func (m *Session) ValidForProposal(clusterID uint64) bool {
	...
	if (m.ClientID == NoOPClientID || m.ClientID == EmptyPayloadClientID) &&
		m.SeriesID != NoOPSeriesID {
		return false
	}
...
func (m *Session) ValidForSessionOp(clusterID uint64) bool {
	...
	return m.SeriesID == SeriesIDForRegister ||
		m.SeriesID == SeriesIDForUnregister
}
