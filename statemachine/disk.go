...
// the CreateSnapshot method. Lookup is also allowed when
// the RecoverFromSnapshot or Close methods are being invoked. Invocations to
// the Update, PrepareSnapshot and RecoverFromSnapshot methods are guarded by
...
// the IOnDiskStateMachine instance can continue from its most recent state.
//
...
// machine, IOnDiskStateMachine always has its state machine state kept on
// disk. Its SaveSnapshot method is only invoked on demand when its latest
// state is required to be streamed to a remote node to help the remote node
// to catch up.
type IOnDiskStateMachine interface {
	...
	// of the Raft log it has processed, or it returns 0 when the state machine
	// is new.
	//
	// The provided stopc channel is used to notify the Open method that node has
	// been stopped and the Open method can choose to abort.
	//
	...
	//
	// The Index field of the input Entry slice is the Raft log index of each
	// entry, it is IOnDiskStateMachine's responsibility to persist the Index
	// value of the most recent processed entry. This is the index value to be
	// returned by the Open method when the IOnDiskStateMachine instance restarts
	// in the future.
	//
	...
	//
	// Dragonboat ensures that Update() and Close() methods will not be invoked
	...
	//
	// RecoverFromSnapshot can also be invoked when the node's progress is
	// significantly behind its leader.
	...
	// IOnDiskStateMachine visible to the query result of the Lookup() method.
	//
